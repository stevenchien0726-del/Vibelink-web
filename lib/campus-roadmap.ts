...
export function createCampusRoadmap(campus: Campus) {
  ...
  return {
    campus,
    hero: { brand: `Vibelink × ${name}`, title: "Campus RoadMap", lead: "從加入校園，到找到和你同頻的人。", intro: `6 Steps，開始探索你的${shortName}。` },
    steps: [
    ...
    tagInstructions: ["打開 Vibelink，進入 Profile", "找到 @Atomic Tags", `搜尋 ${search}，按下搜尋`, `點開 ${tag} 對應社群`, "點擊「加入」；已加入者可直接繼續探索"],
    ...
    radarInstructions: ["打開 AI Radar", `點擊輸入欄的 @，搜尋並選取 ${tag}`, "輸入你想找的人", "Search", "看看 Radar 找到誰"],
